...
# target colors
targets_color = [ [255, 153, 51], [153, 51, 255], [0, 255, 0] ]
...
class Brick(PygameDrawable):
    ...
    def __init__(self, i: int, j: int, width: int, height: int, xdistance: int, color):
        self.i = i
        self.j = j
        self.width = width
        self.height = height
        ...
        for i in range(0, self.brick_cols):
            for j in range(0, self.brick_rows):
                ...
                if (i,j) in self.targets:
                    idx = self.targets.index((i,j))
                    color = targets_color[idx]
                temp = Brick(i, j, self.brick_width, self.brick_height, self.brick_xdistance, color)
